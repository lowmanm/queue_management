...
export class PipelineService {
  ...
  private initializeDefaultData(): void {
    // Create sample Pipeline: Business A
    const businessA = this.createPipeline({
      name: 'Business A',
      description: 'Client services and research operations',
      allowedWorkTypes: ['SUPPORT', 'RESEARCH', 'TRANSCRIBE'],
    });

    if (businessA.pipeline) {
      // Create queues for Business A
      this.createQueue({
        pipelineId: businessA.pipeline.id,
        name: 'Client Support',
        description: 'General client support requests',
        priority: 1,
      ...
      this.createQueue({
        ...
        priority: 3,
        requiredSkills: ['transcription'],
      });

      // Create routing rules for Business A
      const queues = this.getQueuesByPipeline(businessA.pipeline.id);
      if (queues.length >= 3) {
        this.createRoutingRule({
          pipelineId: businessA.pipeline.id,
          name: 'Support Requests',
          description: 'Route support work types to Client Support queue',
          priority: 1,
          conditions: [
            {
        ...
        this.createRoutingRule({
          pipelineId: businessA.pipeline.id,
          name: 'Research Tasks',
          description: 'Route research work to Research queue',
          priority: 2,
          conditions: [
            {
              id: this.generateId('cond'),
          ...
          conditionLogic: 'AND',
          targetQueueId: queues[1].id,
        });

        // Set default queue
        businessA.pipeline.defaultRouting = {
          behavior: 'route_to_queue',
          defaultQueueId: queues[2].id,
        };
        this.pipelines.set(businessA.pipeline.id, businessA.pipeline);
      }
    }

    // Create sample Pipeline: Business B
    const businessB = this.createPipeline({
      name: 'Business B',
      description: 'Priority clients and audit operations',
      allowedWorkTypes: ['PRIORITY', 'EMAIL', 'AUDIT', 'FOLLOWUP'],
    });

    if (businessB.pipeline) {
      this.createQueue({
        pipelineId: businessB.pipeline.id,
        name: 'Priority Clients',
        description: 'High-priority client requests',
        priority: 1,
        requiredSkills: ['priority-handling', 'escalation'],
      });
      ...
      this.createQueue({
        ...
        priority: 2,
        requiredSkills: ['written-communication'],
      });

      this.createQueue({
        pipelineId: businessB.pipeline.id,
        name: 'Audit',
        description: 'Audit and compliance tasks',
        priority: 3,
      ...
      // Create routing rules for Business B
      const queuesB = this.getQueuesByPipeline(businessB.pipeline.id);
      if (queuesB.length >= 4) {
        this.createRoutingRule({
          pipelineId: businessB.pipeline.id,
          name: 'High Priority',
          description: 'Route priority >= 8 to Priority Clients',
          priority: 1,
          conditions: [
            {
              id: this.generateId('cond'),
          ...
          conditionLogic: 'AND',
          targetQueueId: queuesB[0].id,
        });

        this.createRoutingRule({
          pipelineId: businessB.pipeline.id,
          name: 'Email Work',
          description: 'Route email work type',
          priority: 2,
          conditions: [
            {
              id: this.generateId('cond'),
          ...
          conditionLogic: 'AND',
          targetQueueId: queuesB[1].id,
        });

        this.createRoutingRule({
          pipelineId: businessB.pipeline.id,
          name: 'Audit Work',
          description: 'Route audit work type',
          priority: 3,
          conditions: [
            {
              id: this.generateId('cond'),
          ...
          conditionLogic: 'AND',
          targetQueueId: queuesB[2].id,
        });

        // Set default queue
        businessB.pipeline.defaultRouting = {
          behavior: 'route_to_queue',
          defaultQueueId: queuesB[3].id,
        };
        this.pipelines.set(businessB.pipeline.id, businessB.pipeline);
      }
    }

    this.logger.log(
      `Initialized ${this.pipelines.size} pipelines with ${this.queues.size} queues`
    );
  }
